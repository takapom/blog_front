...
export default function PostCard() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        setIsLoading(true)
        const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'
        const response = await fetch(`${apiUrl}/api/posts/?page=${page}&limit=10`)
        if (!response.ok) {
          throw new Error("投稿の取得に失敗しました")
        }
        const data = await response.json()
        
        // 重複を防ぐために、新しいデータのIDが既存のデータに存在しないものだけを追加
        setPosts(prev => {
  ...
  return (
    <div className={styles.postList}>
      {posts.map((post) => (
        <div key={`${post.id}-${post.created_at}`} className={styles.postCard}>
          <div className={styles.postHeader}>
            <Link href={`/posts/${post.id}`} className={styles.postTitle}>
              {post.title}
            </Link>
